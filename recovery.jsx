...
function RecoveryPanel({ completed, setCompleted, setToast, bookNo, onChangeBookNo, topic, onChangeTopic, onChangeTopicAndBookNo }) {
  ...
  const restoreFromDiskAutosave = async () => {
    ...
    try {
      ...
      // [autosaved-image]를 실제 디스크 이미지 URL로 대체 (IndexedDB 용량/성능 문제 회피)
      const getImageUrl = async (name) => {
        for (const ext of [".png", ".jpg", ".jpeg"]) {
          const u = `/pages/${encodeURIComponent(tName)}/${padBook}권/autosave/${name}${ext}`;
          try {
      ...
      if (snap.comicImg === "[autosaved-image]") {
        ...
        if (curSp) snap.comicImg = (await getImageUrl(String(curSp.leftPage).padStart(3,"0")+"_a")) || snap.comicImg;
      }
      if (snap.illustImg === "[autosaved-image]") {
        const curSp = window.BOOK_SPREADS[snap.currentSpread];
        if (curSp) snap.illustImg = (await getImageUrl(String(curSp.rightPage).padStart(3,"0")+"_b")) || snap.illustImg;
      }
      for (const k of Object.keys(snap.completed || {})) {
        const d = snap.completed[k];
        const sp = window.BOOK_SPREADS[k];
        if (!sp) continue;
        if (d.comicImg === "[autosaved-image]") d.comicImg = (await getImageUrl(String(sp.leftPage).padStart(3,"0")+"_a")) || d.comicImg;
        if (d.illustImg === "[autosaved-image]") d.illustImg = (await getImageUrl(String(sp.rightPage).padStart(3,"0")+"_b")) || d.illustImg;
      }
